...
def answer_seven():
	...
	for index, row in df.iterrows():
		...
		for i in range(0, len(fin)):
			...
			for j in range(0, len(fin)):
				if(abs(comp-float(row[fin[j]])) > max):
					max = abs(comp-float(row[fin[j]]))	
		maxDiffList.append(max);
	df['maxDiff'] = maxDiffList;
	countyName = df.sort_values("maxDiff", ascending=False).reset_index(['CTYNAME']);
	return str(countyName['maxDiff'].argmax())
